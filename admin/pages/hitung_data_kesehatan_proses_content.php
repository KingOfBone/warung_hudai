...
<?php
	...
	$konci_tampil_insert_awal = false;
	$konci_tampil_insert_awal = true;
	
	
	$konci_insert = false;
	$konci_insert = true;
	
	$konci_tampil_insert = false;
	$konci_tampil_insert = true;
		...
		if($i_iterasi > 0) {
			if($status == $jumlah_baris) {
				...
				foreach($kualitas_kerja as $kk) {
					...
					$sql = "select penyakit from hasil_kesimpulan
					inner join data_kesehatan_pegawai
