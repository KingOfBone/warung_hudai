						...
						<tbody>
							<?php
								...
								while($data = mysqli_fetch_assoc($hasil)) {
									$input_ke .= "input_ke != $data[input_ke] AND ";
								}
								
								if($input_ke != '')
									$input_ke = substr($input_ke, 0, -4);
								else {
									$input_ke = "input_ke = 1";
								}
								...
								if($hasil) {
									while($data = mysqli_fetch_assoc($hasil)) {
										...
										<tr class='odd gradeX'>
											...
											<td>$jumlah_baris</td>
											<td><a href='$url_hitung_data_kesehatan_proses&&input_ke=$data[input_ke]'><p class='fa fa-expand '> Hitung data ini </p></a></td>
										</tr>
										";
