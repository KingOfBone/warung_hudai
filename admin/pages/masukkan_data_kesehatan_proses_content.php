	...
	<div class="col-lg-12">
		<?php
			if(empty($data_kosong)) {
				...
				$input_rawat_jalan = array();
				for($i_highestRow=$cell_rawat_jalan[0]; $i_highestRow<=$highestRow; $i_highestRow++) {
					$cell = $objWorksheet->getCellByColumnAndRow($cell_rawat_jalan[1], $i_highestRow)->getValue();
					$cell = strtolower($cell);
					
					if($cell != 'ya' && $cell != 'tidak') {
						$input_rawat_jalan[] = $cell;
					}
				}
				
				if(count($input_rawat_jalan) > 0) {
					$input_rawat_jalan = array_unique($input_rawat_jalan);
					$input_rawat_jalan = array_values($input_rawat_jalan);
					
					//var_dump($input_rawat_jalan);
					
					$rawat_jalan = '';
					foreach($input_rawat_jalan as $irj) {
						$rawat_jalan .= "<input type='hidden' name='rawat_jalan[]' value='$irj'>";					
					}
				}
				...
				$input_rawat_inap = array();
				for($i_highestRow=$cell_rawat_inap[0]; $i_highestRow<=$highestRow; $i_highestRow++) {
					$cell = $objWorksheet->getCellByColumnAndRow($cell_rawat_inap[1], $i_highestRow)->getValue();
					$cell = strtolower($cell);
					
					if($cell != 'ya' && $cell != 'tidak') {
						$input_rawat_inap[] = $cell;
					}
				}
				
				if(count($input_rawat_inap) > 1) {
					$input_rawat_inap = array_unique($input_rawat_inap);
					$input_rawat_inap = array_values($input_rawat_inap);
					
					$rawat_inap = '';
					foreach($input_rawat_inap as $iri) {
						$rawat_inap .= "<input type='hidden' name='rawat_inap[]' value='$iri'>";
					}
				}
				
				$rawat_jalan = !empty($rawat_jalan) ? $rawat_jalan : '';
				$rawat_inap = !empty($rawat_inap) ? $rawat_inap : '';
				...
				if(!empty($data_kosong)) {
					...
					for($i_highestRow=$cell_penyakit[0]; $i_highestRow<=$highestRow; $i_highestRow++) {
						...
						if(!in_array($nama_atribut, $bobot_atribut_penyakit)) {
							//echo "$nama_atribut ada di database <br>";
							$konci_penyakit = true;
							$level = 0;
						}
						else {
							//echo "$nama_atribut tidak ada di database <br>";
					...
					foreach($sql_array as $sa) {
						...
						if($konci_insert) {
							mysqli_query($konek, $sql);
							$last_id_datakesehatanpegawai[] = mysqli_insert_id($konek);
						}
					...
					while($data = mysqli_fetch_assoc($hasil)) {
						...
						$last_id_datakesehatanpegawai[$key_last_id], 
						'$data[Nama]', 
						$rawat_jalan,
						$rawat_inap, 
						$tempat_berobat, 
