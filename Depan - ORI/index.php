			...
			<div class="main-content">			
				<div class='tablenya'>
					...
					<table id="q-graph">
						...
						<?php
							$sql = "
								select id_menu, sum(jumlah) from pesanan where id_menu in (
									SELECT distinct(id_menu) 'id_menu' FROM `pesanan`
								) order by id_menu asc
								limit 5
							";
							
							$sql = "SELECT distinct(id_menu) 'id_menu' FROM `pesanan`";
							$sql = mysqli_query($konek, $sql);
							while($row = mysqli_fetch_assoc($sql)) {
								$sql2 = "select id_menu, sum(jumlah) 'jumlah' from pesanan where id_menu = '$row[id_menu]'";
								$sql2 = mysqli_query($konek, $sql2);
								$row2 = mysqli_fetch_assoc($sql2);
								
								$data[$row2['id_menu']] = $row2['jumlah'];
							}
						...
						<tbody>
							<?php 
								$i = 1;
								foreach($data as $id_menu=>$jumlah) { 
									$sql = "SELECT * FROM `menu` where id_menu = '$id_menu'";
									$sql = mysqli_query($konek, $sql);
									$row = mysqli_fetch_assoc($sql);
							?>
							<tr class="qtr" id="q<?php echo $i?>">
								<th scope="row"><?php echo $row['nama_paket']; ?></th>
								<td class="sent bar" style="height: 80px;"><p><?php echo $jumlah; ?></p></td>
								<td class="paid bar" style="height: 99px;"><p><?php echo $jumlah; ?></p></td>
							</tr>
							<?php 
									$i++;
								} 
